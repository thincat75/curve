...
void FlattenCore::DoFlatten(
    ...
    for (uint64_t i = 0; i < cloneSegmentNum; i++) {
        ...
        for (uint32_t j = 0; j != chunkNum; j++) {
            while (workingChunkNum >= flattenChunkConcurrency_) {
                uint32_t completeChunkNum = 0;
                ret = WaitAsycnFlattenChunkDone(tracker, &completeChunkNum);
            ...
            ret = StartAsyncFlattenChunk(tracker, context);
            if (ret < 0) {
                break;
            }
        }
...
int FlattenCore::StartAsyncFlattenChunk(
...
int FlattenCore::WaitAsycnFlattenChunkDone(
    const std::shared_ptr<FlattenChunkTaskTracker> &tracker,
