...
void CopysetNode::save_snapshot_background(::braft::SnapshotWriter *writer,
    ...
    if (0 == fs_->List(chunkDataApath_, &files)) {
        for (const auto& fileName : files) {
            ...
            ChunkID fid;
            CSErrorCode ret;
            uint64_t virtualid = 0, cloneno = 0;

            fid = DatastoreFileHelper::GetChunkFileID(fileName);
            ret = dataStore_->GetCloneInfo(fid, virtualid, cloneno);
            if ((CSErrorCode::Success == ret) && (0 != cloneno)) {
                LocalCloneMeta cloneMeta;
                cloneMeta.set_cloneno(cloneno);
                cloneMeta.set_virtualid(virtualid);
                LocalFileMeta fileMeta;
                std::string cloneMetaStr;
                cloneMeta.SerializeToString(&cloneMetaStr);
                fileMeta.set_user_meta(cloneMetaStr);
                writer->add_file(filePath, &fileMeta);
            } else {
