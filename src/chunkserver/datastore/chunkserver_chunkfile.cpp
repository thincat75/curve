...
CSErrorCode CSChunkFile::flattenWrite(SequenceNum sn,
    ...
    uint32_t startIndex;
    startIndex = chunkFile->metaPage_.bitmap->NextClearBit(0);
    if (startIndex == Bitmap::NO_POS) {//clone chunk already full, no need to flatten
        return errorCode;
    }
    ...
    if (offset + length < size_) {
        errorCode = CSErrorCode::FlattenAgain;
    }
