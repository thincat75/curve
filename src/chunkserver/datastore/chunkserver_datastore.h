...
class CSDataStore {
 public:
    ...
    void searchChunkForObj (SequenceNum sn, 
                            std::vector<File_ObjectInfoPtr>& objInfos, 
                            uint32_t beginIndex, uint32_t endIndex, 
                            std::unique_ptr<CloneContext>& ctx);

    CSErrorCode ReadByObjInfo (CSChunkFilePtr fileptr, char* buf, ObjectInfo& objinfo);
    ...
    void SplitDataIntoObjs (SequenceNum sn,
                            std::vector<File_ObjectInfoPtr>& objInfos, 
                            off_t offset, 
                            size_t length,
                            std::unique_ptr<CloneContext>& ctx);
