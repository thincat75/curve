...
const (
	dirExample = `$ curve bs clone --srcpath /test --seq 1 --dstpath /test2 --user curve`
)
...
func (fCmd *cloneCommand) Init(cmd *cobra.Command, args []string) error {
	fileName := config.GetBsFlagString(fCmd.Cmd, config.CURVEBS_SRCPATH)
...
func (fCmd *cloneCommand) RunCommand(cmd *cobra.Command, args []string) error {
  ...
  row[cobrautil.ROW_RESULT] = fCmd.Error.Message
...
func (fCmd *cloneCommand) AddFlags() {
	...
	config.AddBsSrcPathRequiredFlag(fCmd.Cmd)
	config.AddBsSeqRequiredFlag(fCmd.Cmd)
